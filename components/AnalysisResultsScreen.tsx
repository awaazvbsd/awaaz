  ...
  return (
    <GlassCard className="p-4">
      ...
      <div className="flex items-center gap-4 mt-2">
        ...
        <button onClick={togglePlay} className="glass-base w-11 h-11 flex-shrink-0 rounded-full flex items-center justify-center hover:bg-purple-primary/20 active:bg-purple-primary/30 active:scale-95 transition-all">
...
const AnalysisResultsScreen: React.FC<AnalysisResultsScreenProps> = ({ analysisData, onNewRecording, onClose, onNext, isTeacherView = false, onChatClick, studentHistory, onReportClick, onDateClick, onSelfReportSubmit }) => {
  ...
  const handlers = useSwipeable({
    onSwipedLeft: () => {
      if (onNext && !isTeacherView) onNext();
    },
    onSwipedRight: () => {
      onClose();
    },
  ...
  const Header = () => (
    <header className="fixed top-0 left-0 right-0 h-[60px] flex items-center justify-between px-4 z-20 max-w-2xl mx-auto">
      <button onClick={onClose} className="glass-base w-11 h-11 rounded-full flex items-center justify-center transition-all hover:bg-purple-primary/20 active:bg-purple-primary/30 active:scale-95">
  ...
  return (
    <div className="min-h-screen w-full bg-background-primary relative overflow-hidden" {...handlers}>
      {/* Beams Background */}
        ...
        >
          ...
          {!isTeacherView && (
            <motion.div className="space-y-4 pt-4" variants={itemVariants}>
              <button className="w-full h-14 rounded-2xl flex items-center justify-center font-bold text-white bg-gradient-to-r from-purple-dark to-purple-primary shadow-lg shadow-purple-dark/30 hover:scale-[1.02] active:scale-[0.98] transition-transform">
                <Bookmark className="w-5 h-5 mr-2" /> Save to History
              </button>
              {onNext && (
                <button onClick={onNext} className="w-full h-14 rounded-2xl flex items-center justify-center font-medium text-white glass-base border-purple-primary border hover:bg-purple-primary/20 active:bg-purple-primary/30 active:scale-[0.98] transition-all">
                  Next: Get Suggestions
                </button>
              )}
              <button onClick={onNewRecording} className="w-full h-14 rounded-2xl flex items-center justify-center font-medium text-white glass-base border-purple-primary border hover:bg-purple-primary/20 active:bg-purple-primary/30 active:scale-[0.98] transition-all">
