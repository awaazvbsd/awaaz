...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Ensure canvas is absolutely positioned, transparent, and clipped to a circle
      glContext.canvas.style.position = 'absolute';
      glContext.canvas.style.top = '0';
      glContext.canvas.style.left = '0';
      glContext.canvas.style.width = '100%';
      glContext.canvas.style.height = '100%';
      glContext.canvas.style.borderRadius = '9999px';
      glContext.canvas.style.backgroundColor = 'transparent';
      glContext.canvas.style.display = 'block';
      glContext.canvas.style.pointerEvents = 'none';
      glContext.canvas.style.opacity = '0';
      ...
      // Observe container size changes to keep WebGL in sync
      ...
      const update = (t: number) => {
        ...
        if (rendererInstance && glContext) {
          ...
          if (!firstFrameShown) {
            glContext.canvas.style.opacity = '1';
  ...
  }, [enableVoiceControl, externalAudioStream]);
